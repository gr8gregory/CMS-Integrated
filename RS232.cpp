...
char COMPORTT[5] = "COM4";
char COMPORTR[5] = "COM3";
int flagDiag = false;
int flagMode = false;
...
void SendAudio() {					//Transmit only
	...
	memcpy_s(buf, sizeof(MssgAud), AppendAudio(iBigBuf), lBigBufSize);	// Append the header and payload to a char buffer to send;
	...
	if (flagDiag) {
		outputToPort(cvtA.convert, sizeof(MssgAud) + 1, 1);
		Sleep(1000);
		purgePort(TCom);
		CloseHandle(TCom);
	}
...
void RecieveAudio() {						// Recieve only fn
	...
	short* msgIn = (short*)calloc(lBigBufSize, sizeof(short));
	if (msgIn == NULL) {
	...
	if (MsgBytes > 0) {
		...
		ptr->Data.Buffer = (short*)calloc((SAMPLES_SEC * RECORD_TIME) + 17, sizeof(short));
		...
		memcpy_s(ptr->Data.Buffer, ((ptr->Data.size) * 2) - 17, cvtA.FRAMEAUD.Aud.audioBuf, sizeof(cvtA.FRAMEAUD.Aud.audioBuf));
